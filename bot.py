...
def process_message(msg: str, mention="") -> str:
    if "INFO" in msg:
        msg += "\n"
    if "DEBUG" in msg:
        msg = ""
...
def read_messages(mention="", odmen_mention=""):
    messages = []
    msg = ""
    ...
    if stdin_content == last_message_content:
    ...
    with open(PATH_TO_LAST_MESSAGE, "w") as fout:
        last_message_content = fout.write(stdin_content)

    for line in stdin_content.split("\n"):
        line = process_message(line, mention=mention)
        msg += line
...
async def on_ready():
    try:
        ...
        messages = read_messages(user1.mention, user2.mention)  # role.mention
        for m in messages:
            await ch.send(m)
